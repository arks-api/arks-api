 ...
 * @author
 */
public class WordCount extends Thread implements Stopwords{
    ...
    public void run()
    {
    	...
    	{
    		set.add(STOPWORDS1[i]);
    	}
    	for(int i = 0 ; i < STOPWORDS2.length ; i++)
    	{
    		set.add(STOPWORDS2[i]);
    	}
        ...
        for (Map.Entry<String, Integer> entry : list) {
            if (!set.contains(entry.getKey()) )
            {
            	...
            	Keyword keyword = new Keyword(entry.getKey(), entry.getValue().toString());
            	l.add(keyword);
            }
        }
		...
		try {
			...
			fileMetadata = new FileMetadata(FileCounterManager.getCounter().toString(), path, path, "25MB", "1", "NO DOMAIN FOUND", l);
