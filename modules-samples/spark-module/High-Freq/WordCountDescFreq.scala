...
object Main {
  ...
  def main(args: Array[String]) 
  {
        ...
        val STOPWORDS = Array("a", "a's", "able", "about", "above",
        ...
        val spark = new SparkContext(new SparkConf().setMaster("local[4]").setAppName("Get Important Keywords As Per Highest Frequency Count"))
	

	//INPUT FILE 
        val file = spark.textFile("/path/to/input/file")

        val words = file.flatMap { line => line.toLowerCase().replaceAll("\\p{Punct}|\\d","").split(" ") }
        

	//FILTER IMPLEMENTATION
        def modifiedfilter(line : String):Boolean = 
        {
            val patterntoskip : java.util.Collection[String] = new java.util.HashSet()	//JAVA API USED HERE
            
            for(i <- 0 to (STOPWORDS.length - 1 ))
            {
              patterntoskip.add(STOPWORDS(i))
            }
            
            
            if(line.isEmpty() || patterntoskip.contains(line))
            {
              return false 
            }
            else
        ...
        val filter = words.filter{modifiedfilter}  //CALLING FILTER FUNCTION
        val mapper = filter.map { x => (x,1) }
        val reducer = mapper.reduceByKey(_ + _)
        val sorted = reducer.sortBy(_._2, ascending = false)	//SORT BY VALUE IF _._2 AND KEY IF _._1 AND FALSE FOR DESCENDING

        //FOR DISPLAYING RESULT RDD ON CONSOLE STDOUT
        //sorted.collect().foreach(println)

	//FOR SAVING RESULT RDD IN A DIRECTORY
        sorted.saveAsTextFile("/path/to/output/directory/name")
